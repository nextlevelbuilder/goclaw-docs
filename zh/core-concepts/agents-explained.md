...
在长时间运行的任务中，GoClaw 会在**循环过程中**触发上下文压缩——而不仅仅是在运行完成后。当提示词 token 超过上下文窗口的 85%（可通过 `MaxHistoryShare` 配置，默认 `0.85`）时，agent 会总结内存中约前 70% 的消息，保留后 30%，然后继续迭代。这防止了上下文溢出而不中止当前任务。

## 自动摘要和记忆刷新

每次对话运行结束后，GoClaw 评估是否需要压缩 session 历史：

- **触发条件**：历史超过 50 条消息，或估计 token 超过上下文窗口的 85%
