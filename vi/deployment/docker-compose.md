...
| `database schema is outdated` | Migration chưa chạy sau khi update | Thêm `GOCLAW_AUTO_UPGRADE=true` vào **file** `.env` (không dùng prefix trước command — compose đọc từ `env_file`), hoặc chạy [upgrade overlay](#docker-composeupgradeyml) trước khi start |
